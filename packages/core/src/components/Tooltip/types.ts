...
export interface StyledTooltipProps<T extends Record<string, unknown> = {}>
  extends BaseStyledTooltipProps {
  /** prop to interpolate the returned value of a `css` call within styled-components */
  styledCss?:
    | FlattenSimpleInterpolation
    // eslint-disable-next-line @typescript-eslint/ban-types
    | FlattenInterpolation<ThemedStyledProps<{}, T>>
    | FlattenInterpolation<ThemedStyledProps<BaseStyledTooltipProps, T>>;
}

export interface TooltipProps<E = HTMLElement>
  extends Pick<StyledTooltipProps, "style" | "styledCss">,
